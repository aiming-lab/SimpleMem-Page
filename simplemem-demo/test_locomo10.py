"""
LoComo10 Dataset Test for AnchorMem System
...
from main import AnchorMemSystem
...
class LoCoMoTester:
    """Test AnchorMem system on LoComo10 dataset"""

    def __init__(self, system: AnchorMemSystem, dataset_path: str, use_llm_judge: bool = False, test_workers: int = None):
        self.system = system
...
"""
    ...
    def _process_single_question(self, qa, qa_idx: int):
        ...
        # Category 5 uses special prompt
        if category == 5:
            # Build category 5 prompt for token counting
            adversarial_answer = qa.adversarial_answer if qa.adversarial_answer else "Unknown answer"
            # Use a representative prompt for token counting
            import random
            options = ["Not mentioned in the conversation", adversarial_answer]
            if random.random() < 0.5:
                options = [options[1], options[0]]
            prompt = f"""Based on the context: {context_str}
Question: {question}
Select the correct answer: {options[0]} or {options[1]}"""
        else:
            prompt = self.system.answer_generator._build_answer_prompt(question, context_str)
        ...
        print(f"  Tokens sent to LLM: {token_count}")
    ...
    def run_test(self, num_samples: int = None, save_results: bool = True, result_file: str = 'locomo10_test_results.json', enable_parallel_questions: bool = False):
        ...
        print("\n" + "="*80)
        print(" AnchorMem LoComo10 Dataset Test".center(80))
        print("="*80 + "\n")
        ...
        # Token statistics
        print(f"\nToken Statistics:")
        print(f"  Average tokens per question: {sum(self.token_counts)/len(self.token_counts):.0f}")
        print(f"  Min tokens: {min(self.token_counts)}")
        print(f"  Max tokens: {max(self.token_counts)}")
        print(f"  Total tokens sent: {sum(self.token_counts)}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Test AnchorMem on LoComo10 dataset')
    ...
    # Create system
    print("Initializing AnchorMem system...")
    system = AnchorMemSystem(clear_db=True)
